<x-app-layout>
    <x-slot name="header">Hair Consultations — {{ $customer->name }}</x-slot>
        ...
        <div class="px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div class="flex gap-4 text-sm">
                    ...
                    <a href="{{ route('salon.profile.edit', $customer) }}" class="text-indigo-600 hover:text-indigo-800">Hair profile</a>
                </div>
                @can('salon-consultations.create')
                    <a href="{{ route('salon.consultations.create', $customer) }}">
            ...
            <div class="bg-white shadow-sm rounded-lg divide-y divide-gray-100">
                @forelse ($consultations as $consultation)
                    <div class="p-6">
                        ...
                        @endif
                        @if ($consultation->color_formula)
                            <p class="text-sm text-gray-700 mt-1"><span class="font-medium">Color formula:</span> {{ $consultation->color_formula }}</p>
                        @endif
