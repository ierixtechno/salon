@php
    ...
    // booking, then back-office concerns (inventory, finance, staff), then the
    // less-frequent admin/reporting areas. "My Workspace" is personal and
    // always last.
    $appointmentsActive = request()->routeIs('appointments.*') || request()->routeIs('waitlist.*');
    ...
    $productsActive = request()->routeIs('products.*') || request()->routeIs('product-categories.*') || request()->routeIs('inventory.*');
    $suppliersActive = request()->routeIs('suppliers.*') || request()->routeIs('purchase-orders.*') || request()->routeIs('purchase-returns.*');
    $catalogActive = $productsActive || $suppliersActive;
    ...
    $commissionActive = request()->routeIs('commission.*');
    $workforceActive = $attendanceActive || $leaveActive || $commissionActive;

    $campaignsActive = request()->routeIs('campaigns.*') || request()->routeIs('campaign-automations.*');
    $segmentsActive = request()->routeIs('customer-segments.*');
    $templatesActive = request()->routeIs('notification-templates.*');
    $marketingActive = $campaignsActive || $segmentsActive || $templatesActive;

    $reportsActive = request()->routeIs('reports.*');

    $branchesActive = request()->routeIs('branches.*') || request()->routeIs('resources.*');
    $employeesActive = request()->routeIs('employees.*');
    $orgSettingsActive = request()->routeIs('settings.organization.*');
    $dataExportActive = request()->routeIs('data-exports.*');
...
<nav class="flex-1 px-3 py-4 space-y-1">
    ...
    @if (Auth::user()->can('viewAny', App\Domain\Core\Models\Appointment::class) || Auth::user()->can('viewAny', App\Domain\Core\Models\Customer::class) || Auth::user()->can('viewAny', App\Domain\Core\Models\Invoice::class))
        <x-nav-group title="Sales & Scheduling" :active="$salesActive">
            ...
            @can('viewAny', App\Domain\Core\Models\Customer::class)
                <a href="{{ route('customers.index') }}" class="{{ $navItemBase }} {{ $customersActive ? $navItemActive : $navItemInactive }}">
                    @if ($customersActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @can('viewAny', App\Domain\Core\Models\Invoice::class)
                <a href="{{ route('invoices.index') }}" class="{{ $navItemBase }} {{ $invoicesActive ? $navItemActive : $navItemInactive }}">
                    @if ($invoicesActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    @if (Auth::user()->can('viewAny', App\Domain\Core\Models\Package::class) || Auth::user()->can('gift-cards.view'))
        <x-nav-group title="Loyalty & Offers" :active="$loyaltyActive">
            ...
            @can('gift-cards.view')
                <a href="{{ route('gift-cards.index') }}" class="{{ $navItemBase }} {{ $giftCardsActive ? $navItemActive : $navItemInactive }}">
                    @if ($giftCardsActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    @if (Auth::user()->can('viewAny', App\Domain\Core\Models\Product::class) || Auth::user()->can('viewAny', App\Domain\Core\Models\Supplier::class))
        <x-nav-group title="Catalog & Inventory" :active="$catalogActive">
            ...
            @can('viewAny', App\Domain\Core\Models\Supplier::class)
                <a href="{{ route('suppliers.index') }}" class="{{ $navItemBase }} {{ $suppliersActive ? $navItemActive : $navItemInactive }}">
                    @if ($suppliersActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    @if (Auth::user()->can('expenses.view') || Auth::user()->can('expense-categories.manage') || Auth::user()->can('cash-register.view'))
        <x-nav-group title="Finance" :active="$financeActive">
            ...
            @can('cash-register.view')
                <a href="{{ route('cash-register.index') }}" class="{{ $navItemBase }} {{ $cashRegisterActive ? $navItemActive : $navItemInactive }}">
    ...
    @if (Auth::user()->can('attendance.view') || Auth::user()->can('leave.view') || Auth::user()->can('leave-types.manage') || Auth::user()->can('commission.manage') || Auth::user()->can('commission.view'))
        <x-nav-group title="Workforce" :active="$workforceActive">
            @can('attendance.view')
                <a href="{{ route('attendance.index') }}" class="{{ $navItemBase }} {{ $attendanceActive ? $navItemActive : $navItemInactive }}">
                    @if ($attendanceActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @if (Auth::user()->can('leave.view') || Auth::user()->can('leave-types.manage'))
                <a href="{{ Auth::user()->can('leave.view') ? route('leave.index') : route('leave-types.index') }}" class="{{ $navItemBase }} {{ $leaveActive ? $navItemActive : $navItemInactive }}">
                    @if ($leaveActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @if (Auth::user()->can('commission.manage') || Auth::user()->can('commission.view'))
                <a href="{{ Auth::user()->can('commission.manage') ? route('commission.rules') : route('commission.ledger') }}" class="{{ $navItemBase }} {{ $commissionActive ? $navItemActive : $navItemInactive }}">
                    @if ($commissionActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    @if (Auth::user()->can('marketing.campaigns.view') || Auth::user()->can('marketing.templates.manage') || Auth::user()->can('marketing.segments.manage'))
        <x-nav-group title="Marketing" :active="$marketingActive">
            @can('marketing.campaigns.view')
                <a href="{{ route('campaigns.index') }}" class="{{ $navItemBase }} {{ $campaignsActive ? $navItemActive : $navItemInactive }}">
                    @if ($campaignsActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @can('marketing.segments.manage')
                <a href="{{ route('customer-segments.index') }}" class="{{ $navItemBase }} {{ $segmentsActive ? $navItemActive : $navItemInactive }}">
                    @if ($segmentsActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @can('marketing.templates.manage')
                <a href="{{ route('notification-templates.index') }}" class="{{ $navItemBase }} {{ $templatesActive ? $navItemActive : $navItemInactive }}">
                    @if ($templatesActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    @can('reports.view')
        <x-nav-group title="Reports" :active="$reportsActive">
            <a href="{{ route('reports.sales') }}" class="{{ $navItemBase }} {{ $reportsActive ? $navItemActive : $navItemInactive }}">
                @if ($reportsActive)
                    <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                @endif
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    @if (Auth::user()->can('viewAny', App\Domain\Core\Models\Branch::class) || Auth::user()->can('viewAny', App\Domain\Core\Models\EmployeeProfile::class) || Auth::user()->can('tenant.settings.manage') || Auth::user()->can('data-export.request'))
        <x-nav-group title="Organization" :active="$organizationActive">
            @can('viewAny', App\Domain\Core\Models\Branch::class)
                <a href="{{ route('branches.index') }}" class="{{ $navItemBase }} {{ $branchesActive ? $navItemActive : $navItemInactive }}">
                    @if ($branchesActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @can('viewAny', App\Domain\Core\Models\EmployeeProfile::class)
                <a href="{{ route('employees.index') }}" class="{{ $navItemBase }} {{ $employeesActive ? $navItemActive : $navItemInactive }}">
                    @if ($employeesActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @can('tenant.settings.manage')
                <a href="{{ route('settings.organization.edit') }}" class="{{ $navItemBase }} {{ $orgSettingsActive ? $navItemActive : $navItemInactive }}">
                    @if ($orgSettingsActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @can('data-export.request')
                <a href="{{ route('data-exports.index') }}" class="{{ $navItemBase }} {{ $dataExportActive ? $navItemActive : $navItemInactive }}">
                    @if ($dataExportActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-pink-400"></span>
                    @endif
                    <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
{{-- Not gated by module/feature/plan — every tenant, on every
     subscription plan, gets this. Hidden once already running as an
     installed app (no point re-installing); otherwise triggers the
     real browser install prompt where one is available (captured in
     resources/js/app.js), or shows manual instructions where it isn't
