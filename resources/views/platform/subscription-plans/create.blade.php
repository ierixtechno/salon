<x-platform-layout>
    <x-slot name="header">New Subscription Plan</x-slot>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
        <form method="POST" action="{{ route('platform.subscription-plans.store') }}" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Stable machine-readable identifier, e.g. "salon-beauty-monthly". Letters, numbers, dashes/underscores.</p>
                ...
                <div>
                    ...
                    <select id="billing_interval" name="billing_interval" required class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                        @foreach (['trial', 'monthly', 'yearly'] as $interval)
                            <option value="{{ $interval }}" @selected(old('billing_interval') === $interval)>{{ ucfirst($interval) }}</option>
                        @endforeach
                    </select>
                    ...
                    @foreach ($features as $feature)
                        <label class="flex items-start gap-2 border border-gray-200 rounded-lg px-3 py-2.5 cursor-pointer hover:bg-gray-50 has-[:checked]:border-indigo-400 has-[:checked]:bg-indigo-50">
                            <input type="checkbox" name="features[]" value="{{ $feature->code }}"
                                ...
                                @checked(in_array($feature->code, old('features', [])))>
                            <span>
                    ...
                    @foreach ($modules as $module)
                        <label class="flex items-start gap-2 border border-gray-200 rounded-lg px-3 py-2.5 cursor-pointer hover:bg-gray-50 has-[:checked]:border-indigo-400 has-[:checked]:bg-indigo-50">
                            <input type="checkbox" name="modules[]" value="{{ $module->code }}"
                                class="mt-0.5 rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                @checked(in_array($module->code, old('modules', [])))>
