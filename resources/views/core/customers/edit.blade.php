        ...
        <div class="px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @else
                @php
                    ...
                    $tabs['notes'] = 'Notes';
                    $tabs['programs'] = 'Packages & Programs';
                    if ($showDanger) {
                        $tabs['danger'] = 'Danger Zone';
                    }

                    $defaultTab = $errors->has('body') ? 'notes' : 'profile';
                @endphp

                <x-tabs :items="$tabs" :default="$defaultTab">
                    ...
                    @if ($showConsultations)
                        <x-tab-panel name="consultations">
                            <div class="bg-white shadow-sm rounded-lg p-6">
                    ...
                    <x-tab-panel name="programs">
                        <div class="bg-white shadow-sm rounded-lg p-6">
                            <h3 class="font-medium text-gray-900 mb-4">Packages &amp; Programs</h3>
                            <div class="flex flex-wrap gap-3">
                                @can('viewAny', App\Domain\Core\Models\Package::class)
                                    <a href="{{ route('customers.packages.index', $customer) }}" class="text-sm px-3 py-1.5 rounded-md border bg-gray-50 border-gray-300 text-gray-700 hover:bg-gray-100">Packages</a>
                                @endcan
                                @can('viewAny', App\Domain\Core\Models\MembershipPlan::class)
                                    <a href="{{ route('customers.memberships.index', $customer) }}" class="text-sm px-3 py-1.5 rounded-md border bg-gray-50 border-gray-300 text-gray-700 hover:bg-gray-100">Memberships</a>
                                @endcan
                                @can('wallet.view')
                                    <a href="{{ route('customers.wallet.show', $customer) }}" class="text-sm px-3 py-1.5 rounded-md border bg-gray-50 border-gray-300 text-gray-700 hover:bg-gray-100">Wallet</a>
                                @endcan
                                @can('loyalty.view')
                                    <a href="{{ route('customers.loyalty.show', $customer) }}" class="text-sm px-3 py-1.5 rounded-md border bg-gray-50 border-gray-300 text-gray-700 hover:bg-gray-100">Loyalty</a>
                                @endcan
                            </div>
                        </div>
                    </x-tab-panel>
