            ...
            @if (! $branch)
                <div class="bg-white shadow-sm rounded-lg p-8 text-center">
                    ...
                    @can('viewAny', App\Domain\Core\Models\Branch::class)
                        <a href="{{ route('branches.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white text-sm rounded-md hover:bg-gray-700">
                            + Add a branch
                        </a>
                    @endcan
                </div>
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    @can('appointments.view')
                        <div class="bg-white shadow-sm rounded-lg p-5 flex items-start gap-4">
                            <div class="shrink-0 rounded-lg bg-blue-50 text-blue-600 p-2.5">
                            ...
                            <div>
                                <p class="text-2xl font-semibold text-gray-900">{{ $appointmentCounts['upcoming'] }}</p>
                                <p class="text-sm text-gray-500">Upcoming today</p>
                                <p class="text-xs text-gray-400 mt-0.5">{{ $appointmentCounts['completed'] }} completed so far</p>
                            </div>
                        </div>
                    @endcan
                    ...
                    @can('invoices.view')
                        <div class="bg-white shadow-sm rounded-lg p-5 flex items-start gap-4">
                            <div class="shrink-0 rounded-lg bg-green-50 text-green-600 p-2.5">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            ...
                            <div>
                                <p class="text-2xl font-semibold text-gray-900">{{ number_format((float) $todaysSales, 2) }}</p>
                                <p class="text-sm text-gray-500">Sales today</p>
                                <a href="{{ route('invoices.index', ['branch_id' => $branch->id]) }}" class="text-xs text-indigo-600 hover:text-indigo-800 mt-0.5 inline-block">View invoices &rarr;</a>
                            </div>
                        </div>
                    @endcan
                    ...
                    @can('customers.view')
                        <div class="bg-white shadow-sm rounded-lg p-5 flex items-start gap-4">
                            <div class="shrink-0 rounded-lg bg-purple-50 text-purple-600 p-2.5">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
                                </svg>
                            </div>
                            <div>
                                <p class="text-2xl font-semibold text-gray-900">{{ $newCustomersThisMonth }}</p>
                    ...
                    @can('inventory.view')
                        <div class="bg-white shadow-sm rounded-lg p-5 flex items-start gap-4">
                            <div class="shrink-0 rounded-lg {{ $lowStockItems->isNotEmpty() ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400' }} p-2.5">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            ...
                            <div>
                                ...
                                <a href="{{ route('inventory.index', ['branch_id' => $branch->id]) }}" class="text-xs text-indigo-600 hover:text-indigo-800 mt-0.5 inline-block">View stock &rarr;</a>
                            </div>
                        </div>
                    @endcan
                </div>
