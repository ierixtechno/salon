<x-app-layout>
    <x-slot name="header">New Skin Consultation — {{ $customer->name }}</x-slot>

    <div class="py-8">
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg p-6">
                @if ($branches->isEmpty())
                    <p class="text-sm text-gray-500">No branch has the Beauty Parlour module enabled yet.</p>
                @else
                    <form method="POST" action="{{ route('beauty.consultations.store', $customer) }}" class="space-y-4">
                        @csrf
                        ...
                        <div>
                            <x-input-label for="concerns" value="Skin concerns" />
                        ...
                        <div>
                            ...
                            <textarea id="treatment_plan" name="treatment_plan" rows="2"
                                class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('treatment_plan') }}</textarea>
