...
**Status:** PARTLY SUPERSEDED — package and membership sales are now billed on a normal GST invoice (see "Update" below); gift cards are unchanged.
**Update:** The business asked that assigning a package or membership to a customer always generate an invoice. `SellPackageToCustomer`/`SellMembershipToCustomer` now call `CreateSaleInvoice`: a finalized, sequentially numbered invoice with one line for the package/membership, GST at the package's/plan's own `tax_rate_percent` (default 0, entered on the package/plan form), paid in full with the chosen method. `customer_packages`/`customer_memberships` carry `invoice_id`; `price_paid` is the tax-inclusive amount collected. A zero-priced sale creates no invoice. Sales made before this change keep no invoice and appear in the Payment Ledger as "(no invoice)". Whether GST is due at sale or at redemption remains a question for the tenant's accountant.
